...
The reason rocksdb is much better is because rocksdb was configured to first load all the data in L0 with compactions switched off. And then rocksdb made a single pass over the data to merge-sort all the files in L0 into sorted files in L1. The reason leveldb is very slow is because of high write amplification. Here are the command(s) for loading the data into rocksdb
...
The reason rocksdb is much faster is primarily because of multi-threaded compactions where multiple threads could be simultaneously compacting (via file-renames) non-overlapping key ranges. Here are the command(s) for loading the database into rocksdb.
